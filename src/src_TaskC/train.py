...
# CAMBIARE: Puntare ai file del Task C
...
# -----------------------------------------------------------------------------
# Kaggle Data Helper (ADATTATO PER TASK C)
# -----------------------------------------------------------------------------
def setup_kaggle_data(download_dir="./data"):
    # CAMBIO CHIAVE: Slug della competizione per il Task C
    COMPETITION_SLUG = "semeval-2026-task-13-subtask-c" 
    task_dir = os.path.join(download_dir, "Task_C") # CAMBIO CHIAVE
    ...
    try:
        ...
        # Scarica tutti i file (train, val, test)
...
# -----------------------------------------------------------------------------
# Training Routine (Nessun cambiamento necessario)
# -----------------------------------------------------------------------------
def train_one_epoch(model, dataloader, optimizer, scheduler, scaler, device, 
    ...
    for step, batch in enumerate(progress_bar):
        ...
        torch.cuda.empty_cache() # Pulizia frequente
...
# -----------------------------------------------------------------------------
# Main Execution (ADATTATO PER TASK C)
# -----------------------------------------------------------------------------
if __name__ == "__main__":
    load_dotenv()
    # CAMBIO BANNER
    ConsoleUX.print_banner("SemEval 2026 Task 13 - Subtask C (4-Class Optimized)")
    ...
    # Path discovery (Assumendo che i file si chiamino train.parquet e validation.parquet)
    ...
    experiment = Experiment(
        ...
        project_name=os.getenv("COMET_PROJECT_NAME", "semeval-task13-subtaskc"), # CAMBIO PROJECT NAME
    ...
    for epoch in range(num_epochs):
        ...
        if val_metrics["f1"] > best_f1:
            logger.info(f"⭐ New Best Macro F1: {val_metrics['f1']:.4f}")
            best_f1 = val_metrics["f1"]
            patience_counter = 0
            
            # --- SALVATAGGIO CRITICO AGGIORNATO PER TASK C ---
            save_path = os.path.join(save_dir, "best_model_taskC.pt")
            
            if hasattr(model, "use_lora") and model.use_lora:
                model.base_model.save_pretrained(save_dir)
                
                # Salvataggio di tutte le Custom Heads
                torch.save({
